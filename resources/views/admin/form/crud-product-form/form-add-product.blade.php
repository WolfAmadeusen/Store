...
@section('content')
   ...
   <div class="pt-6">
      ...
      <div class="mx-auto max-w-2xl px-4 py-6 sm:px-6 lg:max-w-7xl lg:px-8">
         <form action="{{ route('admin.product.add') }}" method="POST" class="space-y-6">
            @csrf
            <!-- Title -->
            <div>
            ...
            <div class="grid grid-cols-2 gap-6">
               <div>
                  ...
                  <input type="number" name="price" id="price" value="{{ old('price') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
               </div>
               <div>
                  <label for="quantity" class="block text-sm font-medium text-gray-700">Количество</label>
                  <input type="number" name="quantity" id="quantity" value="{{ old('quantity') }}" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
               </div>
            </div>
